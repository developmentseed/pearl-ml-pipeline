...
NLCD_CLASSES = [
    ...
    90,
    95,
]  # 16 classes + 1 nodata class ("0"). Note that "12" is "Perennial Ice/Snow" and is not present in Maryland.
...
NLCD_IDX_COLORMAP = {idx: NLCD_CLASS_COLORMAP[c] for idx, c in enumerate(NLCD_CLASSES)}
...
LC_COLORMAP = {idx: LC_CLASS_COLORMAP[c] for idx, c in enumerate(LC_CLASSES)}


LC_TREE_COLORMAP = {
    idx: LC_CLASS_TREE_COLORMAP[c] for idx, c in enumerate(LC_CLASSES_TREE)
}
...
NLCD_CLASS_TO_IDX_MAP = (
    get_nlcd_class_to_idx_map()
)  # I do this computation on import for illustration (this could instead be a length 96 vector that is hardcoded here)
...
def fit(model, device, data_loader, num_batches, optimizer, criterion, epoch, memo=""):
    ...
    for batch_idx, (data, targets) in tqdm(
        enumerate(data_loader), total=num_batches, file=sys.stdout
    ):
        data = data.to(device)
